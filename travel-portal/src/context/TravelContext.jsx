...
export function TravelProvider({ children }) {
    ...
    return s ? JSON.parse(s) : [
      {
        ...
        budget: 2500, spent: 1240, status: 'upcoming',
      ...
      },
      {
        ...
        budget: 1200, spent: 0, status: 'planning',
  ...
  const [packingList, setPackingList] = useState(() => {
    ...
    Object.keys(defaultPackingCategories).forEach(cat => {
      initial[cat] = defaultPackingCategories[cat].items.map((item, i) => ({
        id: `${cat}-${i}`, label: item, checked: false,
      }));
    });
    return initial;
  });
  ...
  const addItineraryDay = (tripId, day) => {
    setTrips(prev => prev.map(t =>
      t.id === tripId
        ? { ...t, itinerary: [...t.itinerary, { ...day, id: Date.now(), day: Number(day.day) }].sort((a, b) => a.day - b.day) }
        : t
    ));
  };
  ...
  const deleteItineraryDay = (tripId, dayId) => {
    setTrips(prev => prev.map(t =>
      t.id === tripId ? { ...t, itinerary: t.itinerary.filter(d => d.id !== dayId) } : t
    ));
  };
  ...
  const addExpense = (tripId, expense) => {
    setTrips(prev => prev.map(t => {
      ...
      return { ...t, spent: t.spent + Number(expense.amount), expenses: [...t.expenses, { ...expense, id: Date.now() }] };
    }));
  };
  ...
  const deleteExpense = (tripId, expenseId) => {
    setTrips(prev => prev.map(t => {
      ...
      return {
        ...t,
        spent: Math.max(0, t.spent - (exp ? Number(exp.amount) : 0)),
  ...
  const addPackingCategory = (label, icon) => {
    ...
    if (!key) return;
    setPackingCategories(prev => ({ ...prev, [key]: { label, icon, items: [] } }));
    setPackingList(prev => ({ ...prev, [key]: [] }));
  };

  const deletePackingCategory = (key) => {
    setPackingCategories(prev => { const c = { ...prev }; delete c[key]; return c; });
    setPackingList(prev => { const c = { ...prev }; delete c[key]; return c; });
  };
  ...
  return (
    <TravelContext.Provider value={{
      ...
      togglePacking, addPackingItem, deletePackingItem,
      clearCheckedPackingItems, checkAllPackingItems,
      addPackingCategory, deletePackingCategory,
