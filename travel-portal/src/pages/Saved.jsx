    ...
    <div className="max-w-6xl mx-auto px-6 py-8 pb-16 space-y-8">
      ...
      {saved.length === 0 ? (
        <div className="glass rounded-3xl p-16 text-center border border-white/8">
          ...
          <button onClick={() => setPage('explore')} className="px-6 py-3 bg-accent-light text-accent border border-accent-border rounded-xl font-medium hover:bg-accent/20 transition-all cursor-pointer">
            Browse Destinations
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {saved.map((dest, i) => (
            <div key={dest.id} className="glass rounded-2xl overflow-hidden card-hover border border-white/8 animate-scale-in" style={{ animationDelay: `${i * 100}ms` }}>
              <div className="dest-card" style={{ height: '220px' }} onClick={() => { setSelectedDest(dest); setPage('explore'); }}>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 z-10">
                  <p className="text-accent text-xs flex items-center gap-1 mb-0.5"><MapPin size={10} />{dest.country}</p>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-accent text-xs font-bold">₹{dest.price}</span>
                </div>
                <button
                  onClick={() => setPage('trips')}
                  className="w-full mt-3 py-2 bg-accent-light text-accent border border-accent-border rounded-xl text-xs font-semibold hover:bg-accent/20 transition-all cursor-pointer"
                >
                  Plan a Trip Here
                </button>
              </div>
